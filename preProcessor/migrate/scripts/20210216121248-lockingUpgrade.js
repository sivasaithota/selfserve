...
module.exports = {
  async up({ app, pgClient, filer }) {
    let sql = await filer.readFile(path.resolve(__dirname, '../../sqlScripts/locking/lock_scenario_fn.sql'));
    await pgClient.executeQuery(app._id, sql);
    sql = await filer.readFile(path.resolve(__dirname, '../../sqlScripts/locking/check_lock_fn.sql'));
    await pgClient.executeQuery(app._id, sql);
  },
...
BEGIN
  ...
  IF EXISTS(SELECT 1 FROM "projects" p WHERE p."id"=scenarioId) THEN
    ...
    SELECT "created_by" INTO lock_username FROM "locking" l WHERE l."scenario_id" = scenarioId;
    ...
    ELSE
      ...
      INSERT INTO "locking"("scenario_id","created_by", "updated_by") VALUES(scenarioId, user_name, user_name) RETURNING "id" INTO lockingId;
      ...
      IF (l_id > 0 ) THEN
        ...
        IF ( l_username <> user_name ) THEN
          ...
          IF ((locking_time +  CONCAT(lock_timeout, ' seconds')::INTERVAL) < now() AND (l_explicit_lock IS FALSE OR l_explicit_lock IS NULL)) THEN
            ...
            INSERT INTO "locking" ("scenario_id", "username", "explicit_lock") VALUES (scenarioId, user_name, explicitlock);
        ...
        ELSE
          ...
          IF (l_explicit_lock IS TRUE) THEN
