...
module.exports = {
  async up({ app, pgClient, filer }) {
    const sql = await filer.readFile(path.resolve(__dirname, '../../sqlScripts/scenario/table_metainfo_check.sql'));
    return pgClient.executeQuery(app._id, sql);
  },

  down({ app, pgClient }) {
    const sql = `DROP FUNCTION IF EXISTS table_metainfo_check(TEXT, INTEGER);
    ...
    BEGIN
      query := 'SELECT count(*)=4 FROM (
        SELECT cl.oid,cl.relname,cl.relnamespace,ns.nspname FROM 
          ...
          INNER JOIN (SELECT oid, * FROM pg_catalog.pg_namespace) as ns 
