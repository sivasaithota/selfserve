DROP FUNCTION IF EXISTS get_resource_count(INTEGER, JSON);

CREATE OR REPLACE FUNCTION get_resource_count(scenario_id INTEGER, user_info JSON)
...
BEGIN
	...
	IF user_info->>'userId' IS NOT NULL THEN

		query := CONCAT(query, 'INNER JOIN "users_table_accesses" acc ON ldut."id" = acc."table_id" AND acc."user_id"=', user_info->>'userId');
	...
	IF user_info->>'userId' IS NOT NULL THEN
		...
		query := CONCAT(query, 'INNER JOIN "users_visualization_accesses" acc ON ltr."id" = acc."report_id" AND acc."user_id"=', user_info->>'userId');
	...
	IF user_info->>'userId' IS NOT NULL THEN
		...
		query := CONCAT(query, 'INNER JOIN "users_powerbi_accesses" acc ON lpr."id" = acc."powerbi_id" AND acc."user_id"=', user_info->>'userId');
